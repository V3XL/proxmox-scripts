...
migrate_all_guests() {
    ...
    # LXC containers - offline migration
    pvesh get /nodes/$NODE/lxc -o json | jq -r '.[].vmid' | while read CTID; do
        TARGET=$(pvesh get /nodes -o json | jq -r --arg node "$NODE" '
            .[] | select(.node != $node)
            | "\(.node)"' | head -n1)
        [[ -z "$TARGET" ]] && echo "No target for CT $CTID, skipping." && continue
        echo "Migrating CT $CTID -> $TARGET (offline)"
        pct migrate "$CTID" "$TARGET" || echo "Failed to migrate CT $CTID"
    done

    # QEMU VMs - attempt live migration if possible
    pvesh get /nodes/$NODE/qemu -o json | jq -r '.[].vmid' | while read VMID; do
        TARGET=$(pvesh get /nodes -o json | jq -r --arg node "$NODE" '
            .[] | select(.node != $node and .mem != null and .maxmem != null)
            | "\(.node) \(.maxmem - .mem)"' | sort -k2 -nr | tail -n1 | awk '{print $1}')
        [[ -z "$TARGET" ]] && echo "No target for VM $VMID, skipping." && continue
